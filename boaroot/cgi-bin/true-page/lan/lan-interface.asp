...
<% if tcWebApi_get("vid_common", "skyvid","h") = "38" then %>
        function checkNetSegConflict()
        {
          var ConnectStatus = "<% tcWebApi_get("Dongle_Info","ConnectStatus","s") %>";
          if(ConnectStatus == "1")
          {
            /* dongle running, can't set 192.168.0.x */
            var ethIpAddress = document.forms["lanSettingForm"].ethIpAddress.value;

            if (!isEqualSegment(ethIpAddress, "192.168.0.1", "255.255.255.0"))
            {
...
<% end if %>
        ...
        {
<% if tcWebApi_get("vid_common", "skyvid","h") = "38" then %>
            if (CheckForm() == true && checkNetSegConflict() == true)
<% else %>
            if (CheckForm() == true)
<% end if %>
            {
