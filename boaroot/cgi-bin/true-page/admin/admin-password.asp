...
<head>
  ...
  <script language=JavaScript type=text/javascript>
	...
	function CheckForm()
	{
        ...
        {
        	var reg_surper = /^.*(?=.{15,64})(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[\!\@\#\$\%\^\&\*\(\)\_\+\{\}\:\|\<\>\?\`\-\[\]\,\.\/]).*$/;
			//surper用户的密码需满足至少一个大写、小写字母、数字和特殊字符
			if(!reg_surper.test(document.ConfigForm.newPassword.value))
			{	
				alert("Password can be set at lease 15 Digits to 64 Digits, and the password for superuser needs to contain at least one uppercase letter, one lowercase letter, one number, and one special character.");
				return false;
			}
