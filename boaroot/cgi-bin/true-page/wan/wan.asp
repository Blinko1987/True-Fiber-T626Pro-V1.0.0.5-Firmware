...
if Request_Form("Wan_Flag") = "1" then
  ...
  TCWebApi_set("WanInfo_WanIF","LAN25G","bindlan25")
  end if
        ...
        <% if tcwebApi_get("Vid_EthPortCapability","skylanportenable","h") = "1" then %>
                      <SCRIPT language=JavaScript type=text/javascript>
                        ...
                        if(lan25_exist == 1){
                            ...
                            document.write('<INPUT id="cb_bindlan25" type="checkbox" name="cb_bindlan25" <%if tcWebApi_get("WanInfo_WanIF","LAN25G","h") = "Yes" then asp_Write("checked") end if%> >');
...
<% if tcwebApi_get("WebCustom_Entry","isHsgmiiLanSupported","h") = "Yes" then %>
      aCurBindFlag[26] = "<% tcWebApi_get("WanInfo_WanIF","LAN25G","s") %>";
    for(k=0; k<27; k++)
<% else %>
    for(k=0; k<26; k++)
<% end if %>
