...
<body>
  ...
  <div class="container-full container-resize-height">
    ...
    <div id="display_tree_vis" class="testcss" style="display:none"></div>
  ...
  <SCRIPT language=JavaScript type=text/javascript>
    ...
    {
      var currentTime = new Date(now_time.replace(/-/g,'/')).getTime();
      var connectTime = new Date(access_time.replace(/-/g,'/')).getTime();
    ...
    function convertJsonToTree(json)
    {
      ...
      var map = {};
      json.forEach((item) => {
        map[item.macAddr] = item;
        item.children = [];
      });

      json.forEach((item) => {
        if(item.iscontroller == 1)
        {
          tree = item;
        }
        else
        {
          var parent = map[item.father_mac];
          if (parent)
          {
    ...
    var StaXMap = new Map();
    var StaYMap = new Map();
    function renderNode(startPosition, treeNode)
    {
        if (treeNode.children && treeNode.children.length > 0)
        {
            var blockWidth = 0;
            var blockHeight = nodeHeight + nodeMarginY + nodeHeight;
            ...
            StaYMap.set(treeNode.macAddr.toLowerCase() , startPosition.startY);
            StaXMap.set(treeNode.macAddr.toLowerCase() , startPosition.startX);
            treeNode.positionX=startPosition.startX;
            treeNode.positionY=startPosition.startY;
            return {
    ...
    function shakeTree(treeNode)
    {
        var firstChildCenterStarX = 0, lastChildCenterStartX = 0;

        if(treeNode.children && treeNode.children.length != 0)
        {
          treeNode.children.forEach((e, eindex) => {
              var centerStartX = shakeTree(e);
              ...
              if (eindex === treeNode.children.length - 1)
              {
                  lastChildCenterStartX = centerStartX;
              }
          });
          var newStartX = (lastChildCenterStartX - firstChildCenterStarX) / 2 + firstChildCenterStarX;
          StaXMap.set(treeNode.macAddr.toLowerCase(), newStartX);
          return newStartX;
        }
        else
        {
          StaXMap.set(treeNode.macAddr.toLowerCase(), treeNode.positionX);
          return treeNode.positionX;
        }
    }

    function fitToNetwork(network, data)
    {
      var nodes = data.nodes;
      var boundingBox = {};
      nodes.forEach(function(node)
      {
        var positionX = StaXMap.get(node.MAC.toLowerCase());
        var positionY = StaYMap.get(node.MAC.toLowerCase());
        if (positionX != null && positionY != null)
        {
          if (boundingBox.xMin === undefined || positionX < boundingBox.xMin)
          {
            boundingBox.xMin = positionX;
          }
          if (boundingBox.yMin === undefined || positionY < boundingBox.yMin)
          {
            boundingBox.yMin = positionY;
          }
          if (boundingBox.xMax === undefined || positionX > boundingBox.xMax)
          {
            boundingBox.xMax = positionX;
          }
          if (boundingBox.yMax === undefined || positionY > boundingBox.yMax)
          {
            boundingBox.yMax = positionY;
          }
        }
      });
      ...
      var width = boundingBox.xMax - boundingBox.xMin;
      var height = boundingBox.yMax - boundingBox.yMin;
      if(width < 800)
        width = 800
      if(height < 600)
        height = 600
    ...
    function getStaNodes(nodes,Info)
    {
      if(Info.iscontroller == 0)
      {
        ...
        if(Info.father_mac != "" && Info.linkSpeed != 0)
        {
          ...
          str += ("', x:'"+StaXMap.get(Info.macAddr.toLowerCase()));
          str += ("', y:'"+StaYMap.get(Info.macAddr.toLowerCase()));
          ...
          {
            str += ("', currentSWVersion:'"+ Info.version);
            str += ("', ModelName:'"+ Info.modelName);
    ...
    var nodeMarginY = 18;
    var nodeMarginX = 10;
    function disp_topology_vis_type4()
    {
      ...
      var tree = convertJsonToTree(staInfo);
      renderNode({startX: 0, startY: 0, }, tree);
      shakeTree(tree);
      console.log(tree);
        ...
        if(nodeInfo['Device_Type'] == "agent")
        {
          if(nodeInfo['Connect_Type'] == "Wired")
          {
            ...
            var $ul = "<ul>"
              ...
              +"<li>Port: "+ nodeInfo['Port'] + "</li>"
              +"<li>Online Time: "+ nodeInfo['ConnectTime'] + "</li>";
              if(nodeInfo['currentSWVersion'] != "" && nodeInfo['currentSWVersion'] != "N/A" && nodeInfo['currentSWVersion'] != undefined)
                $ul += ("<li>SW Version: "+ nodeInfo['currentSWVersion'] + "</li>");
              if(nodeInfo['ModelName'] != "" && nodeInfo['ModelName'] != "N/A" && nodeInfo['ModelName'] != undefined)
                $ul += ("<li>Model Name: "+ nodeInfo['ModelName'] + "</li>");
              $ul += "</ul>";
          }
          else
          {
