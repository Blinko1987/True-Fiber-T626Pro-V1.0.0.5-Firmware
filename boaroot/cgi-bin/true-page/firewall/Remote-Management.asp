...
var vEntryIPVERSION = vArrayStr.split(',');
vArrayStr = "<% tcWebApi_get("WanInfo_Common","Wan_ipv6rd","s") %>";
var vEntryipv6rd = vArrayStr.slice(0, -1).split(',');
vArrayStr = "<% tcWebApi_get("WanInfo_Common","Wan_IP6","s") %>";
var vEntryIP6 = vArrayStr.split(',');
vArrayStr = "<% tcWebApi_get("WanInfo_Common","WanValidIF","s") %>";
...
vEntryIPVERSION = SortUtil.SortMyArray( vEntryIPVERSION );
vEntryipv6rd = SortUtil.SortMyArray( vEntryipv6rd );
vEntryIP6 = SortUtil.SortMyArray( vEntryIP6 );
...
var wan_pvc0 = JSON.parse('<% tcWebApi_GetTable("Wan_PVC0", "Entry", "0,7") %>');
var wan_pvc1 = JSON.parse('<% tcWebApi_GetTable("Wan_PVC1", "Entry", "0,7") %>');
var wan_pvc2 = JSON.parse('<% tcWebApi_GetTable("Wan_PVC2", "Entry", "0,7") %>');
var wan_pvc3 = JSON.parse('<% tcWebApi_GetTable("Wan_PVC3", "Entry", "0,7") %>');
var wan_pvc4 = JSON.parse('<% tcWebApi_GetTable("Wan_PVC4", "Entry", "0,7") %>');
var wan_pvc5 = JSON.parse('<% tcWebApi_GetTable("Wan_PVC5", "Entry", "0,7") %>');
var wan_pvc6 = JSON.parse('<% tcWebApi_GetTable("Wan_PVC6", "Entry", "0,7") %>');
var wan_pvc7 = JSON.parse('<% tcWebApi_GetTable("Wan_PVC7", "Entry", "0,7") %>');

var selectwanif = -1;
IPv6_Configuration_Select();
...
function IPv6_Configuration_Select()
{
  var Wan_PD6_val = "-";
  var tr069WAN_index = -1;
  var internetWAN_index = -1;
  var rd6WAN_index = -1;
  var reg = /([0-9]_)(.*?)(_R_VID)/g;

  for(i = 0; i < nEntryNum; i++)
  {
    var mcWanName = matchWanName(reg, vEntryName[i]);
    if (mcWanName != null)
    {
<% if tcWebApi_get("vid_common", "skyvid","h") = "38" then %>
      if ((mcWanName == "TR069_VOICE_INTERNET") || (mcWanName == "TR069_INTERNET"))
<% else %>
      if (mcWanName == "TR069_INTERNET")
<% end if %>
      {
        if(vEntryIPVERSION[i] == "IPv6" || vEntryIPVERSION[i] == "IPv4/IPv6")
        {
          tr069WAN_index = i;
        }
      }
      else if (mcWanName == "INTERNET")
      {
        if(vEntryIPVERSION[i] == "IPv6" || vEntryIPVERSION[i] == "IPv4/IPv6")
        {
          internetWAN_index = i;
        }
        else if(vEntryipv6rd[i] == "Yes")
        {
          rd6WAN_index = i;
        }
      }
  ...
  if(tr069WAN_index != -1 && vEntryIP6[tr069WAN_index] != "N/A" && vEntryIP6[tr069WAN_index] != "" && vEntryIP6[tr069WAN_index] != "-")
  {
    selectwanif = IFIdxArray[tr069WAN_index];
  }
  else
  {
    if(internetWAN_index != -1 && vEntryIP6[tr069WAN_index] != "N/A" && vEntryIP6[tr069WAN_index] != "" && vEntryIP6[tr069WAN_index] != "-")
    {
      selectwanif = IFIdxArray[internetWAN_index];
    }
    else
    {
      if(rd6WAN_index != -1 && vEntryIP6[tr069WAN_index] != "N/A" && vEntryIP6[tr069WAN_index] != "" && vEntryIP6[tr069WAN_index] != "-")
      {
        selectwanif = IFIdxArray[rd6WAN_index];
      }
    }
  }
}

function checkWanPvc(wan_pvc, entry)
{
  var br0IPv6 = "";
  for(var i = 0; i < wan_pvc.length; i++)
  {
    if(wan_pvc[i].EntryIndex == entry)
    {
      if(wan_pvc[i].TrueBr0Addr != "" && wan_pvc[i].TrueBr0Addr != undefined)
        br0IPv6 = wan_pvc[i].TrueBr0Addr.split("/")[0];
      else if(wan_pvc[i].Br0Addr != "" && wan_pvc[i].Br0Addr != undefined)
        br0IPv6 = wan_pvc[i].Br0Addr.split("/")[0];
    }
  }
  return br0IPv6;
}
...
function get_IPv6_Address()
{
  ...
  var pvc_index = Math.floor(selectwanif / 8);
  var entry_index = Math.floor(selectwanif % 8);

  if(pvc_index == 0)
    result = checkWanPvc(wan_pvc0, entry_index);
  else if(pvc_index == 1)
    result = checkWanPvc(wan_pvc1, entry_index);
  else if(pvc_index == 2)
    result = checkWanPvc(wan_pvc2, entry_index);
  else if(pvc_index == 3)
    result = checkWanPvc(wan_pvc3, entry_index);
  else if(pvc_index == 4)
    result = checkWanPvc(wan_pvc4, entry_index);
  else if(pvc_index == 5)
    result = checkWanPvc(wan_pvc5, entry_index);
  else if(pvc_index == 6)
    result = checkWanPvc(wan_pvc6, entry_index);
  else if(pvc_index == 7)
    result = checkWanPvc(wan_pvc7, entry_index);
...
function LoadFrame()
{
  ...
  var Ipv6LanIP_distribute = get_IPv6_Address();
  var AccessIP = "<%tcWebApi_get("WebCurSet_Entry","AccessIp","s")%>";
  var Ipv6LanIP_distribute_temp = "";
  if(Ipv6LanIP_distribute != "" && Ipv6LanIP_distribute != "N/A")
  {
    var index = Ipv6LanIP_distribute.indexOf("/");
    Ipv6LanIP_distribute_temp = Ipv6LanIP_distribute.substring(0,index);
  }
